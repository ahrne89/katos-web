  ...
  const projects = [
    {
      ...
      outcome: 'Zero recruiter touchpoints needed for initial screening.',
      link: 'https://job-agent-sigma.vercel.app',
    },
    {
      number: '02',
          ...
          {projects.map((p, i) => (
            <div key={i} className="two-col card-padding" style={{ background: '#080d1f', padding: '48px', display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '48px', alignItems: 'start' }}>
              ...
              <div>
                ...
                {p.link && (
                  <a href={p.link} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginTop: '20px', padding: '8px 18px', border: '1px solid rgba(123,156,255,0.3)', borderRadius: '8px', fontSize: '12px', color: '#7b9cff', textDecoration: 'none' }}>
                    View live →
                  </a>
                )}
              </div>
