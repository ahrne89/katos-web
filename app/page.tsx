  ...
  return (
    <>
      <style>{`
        ...
        @keyframes heartbeat { 0%,100%{transform:scale(1);}15%{transform:scale(1.08);}30%{transform:scale(1);}45%{transform:scale(1.04);}60%{transform:scale(1);} }
        @keyframes glowRing { 0%,100%{transform:scale(1);opacity:0.35;} 50%{transform:scale(1.4);opacity:0;} }
      ...
      <main style={{ minHeight: '100vh', background: '#080d1f', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '560px', padding: '80px 20px 0', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position: 'relative', width: '200px', height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '8px 0' }}>
            <div style={{ position: 'absolute', width: '100px', height: '100px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(180,160,255,0.2)', animation: 'glowRing 2.5s ease-out infinite' }} />
            <div style={{ position: 'absolute', width: '100px', height: '100px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(180,160,255,0.2)', animation: 'glowRing 2.5s ease-out infinite 0.83s' }} />
            <div style={{ position: 'absolute', width: '100px', height: '100px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(180,160,255,0.2)', animation: 'glowRing 2.5s ease-out infinite 1.66s' }} />
            <div style={{ position: 'absolute', width: '120px', height: '120px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(120,100,255,0.15) 0%, transparent 70%)', animation: 'glowPulse 2.5s ease-in-out infinite' }} />
            <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'radial-gradient(circle at 38% 36%, rgba(255,255,255,0.9) 0%, rgba(200,180,255,0.8) 30%, rgba(120,90,220,0.6) 65%, rgba(60,40,160,0.4) 100%)', animation: 'heartbeat 2.5s ease-in-out infinite', boxShadow: '0 0 24px rgba(160,120,255,0.6), 0 0 48px rgba(120,90,220,0.3)', position: 'relative', zIndex: 5 }} />
          </div>

          {/* Chat messages */}
          {chatOpen && (
            <div ref={chatContainerRef} style={{ width: '100%', maxHeight: '36vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', padding: '4px 0 8px', marginBottom: '8px' }}>
              {messages.map((m, i) => (
